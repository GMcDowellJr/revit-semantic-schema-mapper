...
| `DEPENDS_ON` | Source structurally depends on another element | `Element.GetDependentElements(...)`; `Extrusion.Sketch`/`Blend.BottomSketch`/`.TopSketch`/`Sweep.PathSketch`/`.ProfileSketch`/`SweptBlend.BottomSketch`/`.PathSketch`/`.TopSketch`/`Revolution.Sketch -> Sketch` (ends-with `Sketch(Id)?` keyword, confirmed against a live 2024 crawl: 10 edges, 5 distinct source types, zero counterexamples -- deliberately an ends-with match, not a bare substring, so it doesn't also catch `SketchPlane`, handled separately below, or unrelated coincidences like `View.GetSketchyLines`; the optional `Id` suffix, added later, covers the `ElementId`-returning form of the same relationship, e.g. `Toposolid.SketchId`/`FabricSheet.SketchId` -- doesn't collide with `SketchPlane`/`SketchPlaneId` since `"SketchPlaneId".endswith("SketchId")` is `False`) |
| `REFERENCES` | Generic reference that doesn't fit a more specific type but has a resolvable target concept not yet worth its own edge type | `Element.Document`/`FailuresAccessor.GetDocument() -> Document` (confirmed against a live 2024 crawl: 19 edges, 19 distinct source types, zero counterexamples); `BIMExportOptions.ViewId`/`ElevationMarker.GetViewId() -> View` (same crawl: 12 edges, 12 distinct source types, zero counterexamples); `FamilyInstance.Room`/`.FromRoom`/`.ToRoom`/`Document.GetRoomAtPoint() -> Autodesk.Revit.DB.Architecture.Room` (same crawl: 7 edges, 4 distinct source types, 3 of 7 independently corroborated by RevitLookup); `Entity.Schema`/`Field.Schema`/`Field.SubSchema -> Autodesk.Revit.DB.ExtensibleStorage.Schema` (exact-match keyword, not a bare substring -- `Schema.ListSchemas`/`.Lookup` are a different, static-utility pattern that a substring match would have incorrectly swept up too); `CurveByPoints.SketchPlane`/`CurveElement.SketchPlane`/`Sketch.SketchPlane`/`View.SketchPlane -> SketchPlane` (exact-match keyword, checked before the `DEPENDS_ON` `Sketch$` rule above since `SketchPlane` is a distinct real type, not a kind of `Sketch`, even though its name ends in a way that would otherwise collide; same crawl: 4 edges, 4 distinct source types, zero counterexamples); `Control.View`/`Dimension.View`/`Options.View`/`SpatialElementTag.View`/`Events.ViewPrintedEventArgs.View`/`Events.ViewPrintingEventArgs.View -> View` (exact-match keyword, confirmed against a real crawl's candidate_edges.json: 6 edges, 6 distinct source types, all already `direct_return_type` confidence via the crawled `View` type, zero counterexamples -- this rule only upgrades the edge_type from `UNKNOWN_DB_OBJECT_REFERENCE` to `REFERENCES`, distinct from the pre-existing `ViewId`/`GetViewId` rule which fires off an `ElementId`-typed member instead of a direct `View`-typed one); `AssemblyInstance.Location`/`Element.Location`/`FamilyInstance.Location`/`Group.Location`/`ModelText.Location`/`SpatialElement.Location`/`SpatialElementTag.Location -> Location` (exact-match keyword, confirmed against the same crawl: 7 edges, 7 distinct source types -- `Element` is the base declaring property, the rest are overrides of it; `AssemblyInstance`'s docs describe it as "used to find the physical location of the assembly instance" -- zero counterexamples); `Element.GetExternalResourceReference`/`ExternalResourceLoadData.GetExternalResourceReference`/`LinkLoadResult.GetExternalResourceReference() -> ExternalResourceReference` (exact-match keyword, confirmed against the same crawl: 3 edges, 3 distinct source types, zero counterexamples -- the method name already spells out its own return type, self-confirming the same way `GetDocument`/`SketchPlane` are); `Connector.ConnectorManager`/`FabricationPart.ConnectorManager`/`MEPCurve.ConnectorManager`/`MEPModel.ConnectorManager`/`MEPSystem.ConnectorManager`/`Structure.Hub.GetHubConnectorManager -> ConnectorManager` (ends-with keyword `ConnectorManager$`, broadened from an initial exact-match version after a follow-up pareto re-run showed it left the `Hub` edge behind every year; per docs/crawl_notes.md's "2026-07-09 unknown-edge Pareto review" and its "2026-07-09 follow-up" entry, stable across a real 2024/2025/2026 crawl comparison: 6 edges/6 distinct source types every year, zero counterexamples once all 6 are accounted for); `BuildingPadType.ThermalProperties`/`CeilingType.ThermalProperties`/`FloorType.ThermalProperties`/`RoofType.ThermalProperties`/`WallType.ThermalProperties -> ThermalProperties` (exact-match keyword; same three-year comparison, a complete 5-edge/5-distinct-source-type cluster every year, zero counterexamples); `Rebar.GetReinforcementRoundingManager`/.../`ReinforcementSettings.GetRebarRoundingManager -> RebarRoundingManager` and `FabricArea.GetReinforcementRoundingManager`/.../`ReinforcementSettings.GetFabricRoundingManager -> FabricRoundingManager` (ends-with keyword `RoundingManager$`, `target_hint=None` since the one naming pattern covers two distinct real target types disambiguated only by the verified return type; same three-year comparison, two complete clusters -- 5 and 4 edges respectively -- stable every year, zero counterexamples); `CurveByPoints.Subcategory`/`ModelCurve.Subcategory`/`SymbolicCurve.Subcategory -> GraphicsStyle` (exact-match keyword, checked ahead of the generic `Category` keyword rule below since `"Subcategory"` contains `"Category"` as a substring and would otherwise hit the target-conflict check and fall back to `UNKNOWN_DB_OBJECT_REFERENCE` -- Revit represents a subcategory as a `GraphicsStyle` object, confirmed by the return type itself; same three-year comparison, a complete 3-edge/3-distinct-source-type cluster every year, zero counterexamples); `Category.GetGraphicsStyle -> GraphicsStyle` plus `GeometryObject.GraphicsStyleId`/`SolidOptions.GraphicsStyleId`/`TessellatedShapeBuilder.GraphicsStyleId -> GraphicsStyle` (substring keyword `GraphicsStyle`, covering both the direct-return and `ElementId` forms of the same relationship; same three-year comparison, complete 4-edge and 3-edge clusters respectively, stable every year, zero counterexamples). Room and Schema's real fully-qualified names are outside the base `Autodesk.Revit.DB` namespace (`.Architecture`/`.ExtensibleStorage` respectively) -- see `_NON_DB_NAMESPACE_TARGETS` in classify.py for why they need an explicit override rather than the usual bare-short-name target_hint. |
