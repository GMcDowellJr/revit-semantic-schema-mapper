...
| `--cross-validate-dll MANIFEST_PATH` | off | Stage B of DLL reflection cross-validation (`docs/dll_reflection_v0.md`): cross-check an existing `--output-dir`'s `node_type_candidates.json`/`candidate_edges.json` against a `ground_truth_manifest_<version>.json` (Stage A's output, produced separately by `reflect_revit_api.ps1` on a Windows machine with Revit installed). Writes `ground_truth_report.json`, updates the two candidate files in place with each candidate's `dll_*` fields, and refreshes a summary section -- no crawling, fetching, or live Revit/DLL access |
| `--cross-validate-revitlookup REFERENCE_PATH` | off | Stage C of DLL reflection cross-validation (`docs/dll_reflection_v0.md`): cross-check an existing `--output-dir`'s `candidate_edges.json` against a `revitlookup_reference_<version>.json` (produced separately by `python -m revit_schema_mapper.revitlookup`, which mines RevitLookup's own public C# descriptor source -- no Revit/RevitLookup/DLL access). Writes `revitlookup_cross_validation_report.json`, updates `candidate_edges.json` in place with each edge's `revitlookup_*` fields, and refreshes a summary section. Can run before, after, or without `--cross-validate-dll` -- the two never touch each other's fields |
