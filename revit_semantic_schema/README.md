...
*not* a Room-specific property either; it's inherited from `Autodesk.Revit.DB.Architecture
.SpatialElement`, an intermediate base class between `Room` and `Element`
(`Room : SpatialElement : Element`). So `Name` and `Number` reach the object model through the
*same* mechanism (an inherited base property), just at different levels of the inheritance
chain, not two different mechanisms as originally guessed. They're still correctly kept as two
distinct concepts (see `docs/crawl_notes.md` for how the known-edge report resolves this).
